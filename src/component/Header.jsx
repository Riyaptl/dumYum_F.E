...
const Header = () => {
  ...
  return (
    <nav className={`fixed top-0 left-0 w-full bg-white shadow-md z-50 ${scrolled ? 'scrolled' : ''}`}>
      <div className="wrapper bg-white text-black md:px-4 py-5 flex justify-between items-center md:max-w-[90%] w-full mx-auto h-full">
        ...
        <div className="flex items-center pl-4">
          {showMenu ? (
            <FaTimes onClick={handleMenuClick} className="btn close-btn text-gray-700 text-xl lg:hidden" />
          ) : (
            <FaBars onClick={handleMenuClick} className="menu-icon cursor-pointer lg:hidden text-gray-700 text-xl" />
          )}
          <ul className={showMenu ? 'nav-links absolute top-full left-0 w-full bg-white p-4' : 'nav-links hidden lg:flex'}>
            {jsonData.menuItems.map((menuItem, index) => (
              <li key={index} className="relative">
                <Link to={menuItem.url} className="text-text-black  py-2 px-4 block" onMouseEnter={menuItem.title === 'Products' ? handleProductsHover : menuItem.title === 'Meet the Master' ? handleMeetTheMasterHover : null} onMouseLeave={handleDropdownLeave}>
                  {menuItem.title}
                </Link>
                {menuItem.isMegaMenu && showProductsDropdown && (
                  <div className="mega-box bg-white shadow-lg md:shadow-none absolute top-full left-0 right-0 md:transform md:translate-y-1/5 md:-translate-x-1/3 w-full lg:w-[60vw] mx-auto p-4 z-20" onMouseEnter={handleProductsHover} onMouseLeave={handleDropdownLeave}>
                    <div className="content flex flex-row">
                      <div className="hidden md:block md:w-1/2">
                        <img src={jsonData.megaMenu.imageUrl} alt="Mega Menu Image" className="w-full h-auto" />
                      </div>
                      <div className="row w-full lg:w-1/2 flex justify-evenly ml-4">
                        <div className="">
                          <ul className="mega-links">
                            <header className="text-lg font-semibold mb-2">{jsonData.megaMenu.types[0].title}</header>
                            {categories.map((item, itemIndex) => (
                              <li key={itemIndex}>
                                <Link to={`/${item._id}?page=category`} className="text-gray-900 py-1 block">{item.name}</Link>
                              </li>
                            ))}
                          </ul>
                        </div>
                        <div className="">
                          <ul className="mega-links">
                            <header className="text-lg font-semibold mb-2">{jsonData.megaMenu.types[1].title}</header>
                            {specials.map((item, itemIndex) => (
                              <li key={itemIndex}>
                                <Link to={`/${item._id}?page=special`} className="text-gray-900 hover:text-gray-500 py-1 block">{item.name}</Link>
                              </li>
                            ))}
                          </ul>
                        </div>
            ...
            <li className="relative">
              <button onClick={handleMeetTheMasterHover} className="text-black  py-2 px-4 block focus:outline-none" onMouseEnter={handleMeetTheMasterHover} onMouseLeave={handleDropdownLeave}>
                Meet the Master
              </button>
              {showMeetTheMasterDropdown && (
                <div className="meet-chef-dropdown bg-white shadow-lg md:shadow-none absolute top-full md:right-14 md:transform md:translate-x-1/4 w-full lg:w-[60vw] mx-auto p-4 z-20" onMouseEnter={handleMeetTheMasterHover} onMouseLeave={handleDropdownLeave}>
                  <div className="content flex flex-wrap justify-between flex-col sm:flex-row">
                    <div className="row lg:w-1/2 sm:w-1/3 hidden sm:block">
                      <img src={jsonData.feedback.imageUrl} alt="" className="w-full h-auto" />
                    </div>
